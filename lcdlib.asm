...
;	lcdlib.c:23: LCD_entryModeSet(1, 1); /* increment and no shift */
	mov	dpl, #0x07
	lcall	_LCD_IRWrite
;	lcdlib.c:24: LCD_displayOnOffControl(1, 1, 1); /* display on, cursor on and blinking on */
	mov	dpl, #0x0f
	lcall	_LCD_IRWrite
...
;------------------------------------------------------------
;	lcdlib.c:38: void LCD_functionSet(void) {
;	-----------------------------------------
;	 function LCD_functionSet
;	-----------------------------------------
_LCD_functionSet:
;	lcdlib.c:39: lcd_ready = 0;
	mov	_lcd_ready,#0x00
;	lcdlib.c:42: DB = 0x20;  // DB<7:4> = 0010, <RS,E,x,x>=0
	mov	_P1,#0x20
;	lcdlib.c:43: E = 1;
;	assignBit
	setb	_P1_2
...
;	assignBit
	clr	_P1_2
;	lcdlib.c:45: delay(DELAY_AMOUNT);
	mov	dpl, #0x28
	lcall	_delay
;	lcdlib.c:46: E = 1;
;	assignBit
...
;	assignBit
	clr	_P1_2
;	lcdlib.c:48: delay(DELAY_AMOUNT); // added, to ensure sufficient delay
	mov	dpl, #0x28
	lcall	_delay
;	lcdlib.c:49: DB7 = 1; // 2-line model
;	assignBit
	setb	_P1_7
;	lcdlib.c:51: E = 1;
;	assignBit
...
;	lcdlib.c:58: lcd_ready = 0;
	mov	_lcd_ready,#0x00
;	lcdlib.c:59: DB = (c & 0xf0) | 0x08; //; keep the RS
	mov	a,#0xf0
	anl	a,r7
	orl	a,#0x08
	mov	_P1,a
...
;	assignBit
	clr	_P1_2
;	lcdlib.c:63: DB = (c << 4) | 0x08; // keep the RS
	mov	a,r7
	swap	a
	anl	a,#0xf0
	mov	r7,a
...
;	assignBit
	clr	_P1_2
;	lcdlib.c:66: delay(DELAY_AMOUNT);
	mov	dpl, #0x28
	lcall	_delay
;	lcdlib.c:67: lcd_ready = 1;
	mov	_lcd_ready,#0x01
;	lcdlib.c:68: }
	ret
;------------------------------------------------------------
...
;------------------------------------------------------------
;	lcdlib.c:69: void LCD_write_string(char* str) {
;	-----------------------------------------
...
00101$:
	...
	mov	a,r4
	jz	00104$
;	lcdlib.c:71: LCD_write_char(*str);
	mov	dpl,r5
	mov	dph,r6
	mov	b,r7
	lcall	__gptrget
...
;------------------------------------------------------------
;	lcdlib.c:74: void delay(unsigned char n) {
;	-----------------------------------------
;	 function delay
;	-----------------------------------------
_delay:
